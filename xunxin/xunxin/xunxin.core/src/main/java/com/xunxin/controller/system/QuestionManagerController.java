	...
	public Object full_qa_list() {
		...
		try {
			...
			if(!StringUtils.isBlank(pd.getString("status")) && !StringUtils.trim(pd.getString("status")).equals("")) {
			    query.addCriteria(Criteria.where("status").is(pd.getString("status")));
			}
        ...
        try {
            ...
            if (Arrays.binarySearch(fileExts, fileExt) != -1) {
                ...
                if(fileName.length() > 10) {
                    String subName = fileName.substring(0, 9); 
                    url = "D:\\programmeTools\\nginx-1.12.1\\html\\photo\\qa" + "/" + subName + OrderGeneratedUtils.getOrderNo() + "." + fileExt;
                }else {
                    url = "D:\\programmeTools\\nginx-1.12.1\\html\\photo\\qa" + "/" + OrderGeneratedUtils.getOrderNo() + fileName;
                }
