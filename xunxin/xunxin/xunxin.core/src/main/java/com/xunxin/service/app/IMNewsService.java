...
//				Context context = new Context(new LeaveMessage());        
//			     context.executeStrategy(userId, null, dynamicId, type,sysDictService,xunxinUserDeviceTokenMapper,appUserService);
//			     messageRead.setTag(2);
//			}
//			//审核不通过
//			else if(type != null && type.equals(3)){
//				Context context = new Context(new NotThrough());        
//			     context.executeStrategy(userId, null, dynamicId, type,sysDictService,xunxinUserDeviceTokenMapper,appUserService);
//			     messageRead.setTag(2);
//			}
//			//审核通过
//			else if(type != null && type.equals(4)){
//				Context context = new Context(new ThroughMessage());        
//			     context.executeStrategy(userId, null, dynamicId, type,sysDictService,xunxinUserDeviceTokenMapper,appUserService);
//			     messageRead.setTag(2);
//			}
//			//有人对你实施举报
//			else if(type != null && type.equals(2)){
//				Context context = new Context(new ReportMessage());        
//			     context.executeStrategy(userId, anotherId, null, type,sysDictService,xunxinUserDeviceTokenMapper,appUserService);
//			     messageRead.setTag(2);
//			}
//			//动态点赞
//			else if(type != null && type.equals(5)){
//				Context context = new Context(new GiveUpMessage());        
...
//				Context context = new Context(new CommentMessage());        
//			     context.executeStrategy(userId, anotherId, dynamicId, type,sysDictService,xunxinUserDeviceTokenMapper,appUserService);
//			     messageRead.setTag(3);
//			}
//			//匹配
//			else if(type != null && type.equals(7)){
//				Context context = new Context(new MatchingMessage());        
//			     context.executeStrategy(userId, anotherId, null, type,sysDictService,xunxinUserDeviceTokenMapper,appUserService);
//			     messageRead.setTag(3);
//			}
//			//关注
//			else if(type != null && type.equals(8)){
//				Context context = new Context(new FollowMessage());        
//			     context.executeStrategy(userId, anotherId, null, type,sysDictService,xunxinUserDeviceTokenMapper,appUserService);
//			     messageRead.setTag(3);
//			}
			MessTypeFactory factory = new MessTypeFactory();
...
//	    QuartzManager.removeJob(JOB_NAME, JOB_GROUP_NAME, TRIGGER_NAME, TRIGGER_GROUP_NAME,MyScheduler); 
	...
	public List<Map<String, Object>> findMessageByUserId(Integer userId) {
		...
		if(listOfficial != null && listInteraction.size() > 0 && listOfficial.get(0).getUpdateTime()!=null){
		...
		List<UserFriends> list = userFriendsService.findFriendList(userId);
		Map<String,Object> mapFriends = new HashMap<>();
		mapFriends.put("friends", list);
		List<Map<String, Object>> mapList=new ArrayList<>();
		mapList.add(mapHert);
		mapList.add(mapOfficial);
		mapList.add(mapInteraction);
		mapList.add(mapFriends);
		return mapList;
	}

	public List<Integer> queryMomentPlanB(Integer userId, Integer range, Integer age) {
		NearLocation near =nearLocationService.findOneByQuery(userId);
		List<Integer> idList = arecordService.findMoment(userId);
		List<NearLocation> list = null;
		List<UserEntity> listUser = null;
		if(range == null){
			list = nearLocationService.queryNearLocation(near.getLongitude(),near.getLatitude(),userId,20);
