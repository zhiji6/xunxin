...
<body class="gray-bg">
                    ...
                    <div class="ibox-content">
                        <div id="toolbar" class="btn-group">
                            <div class="pull-left form-inline form-group">
                                ...
                                <select class="form-control" name="isHot" id="isHot" value="${pd.isHot}" style="width:140px;">
                                ...
                                <button type="button" class="btn btn-default btn-danger" onclick="isHot();">
                                     热点
                                </button>
                            </div>
                        </div>
    ...
    <script type="text/javascript">
        ...
        function isHot(){
            var ids = getBstCheckedId('id');
            if((ids.length < 1)){
                layer.msg('请选中信息再进行热点话题操作。');
                return false;
            }
            var idsStr = ids.toString();
            layer.confirm('确定升级已选信息为热点话题吗？', {
                btn: ['确认','取消'],
                shade: false,
                yes: function(index, layero){
                    $.ajax({
                        type: "POST",
                        url: '<%=basePath%>api/qa/qa_hot_topic?tm=' +  new Date().getTime(),
                        data: {
                            IDS: idsStr
                        },
                        dataType: 'json',
        ...
        //是否热点  /**  * 热点:   0:否  1:是  */
        function formatHot(value, row, index){
            var d = '';  
            if(row.isHot == 0){ d = '否'; }
            if(row.isHot == 1){ d = '是'; }
            return d;  
