...
//			int isAuthentication = appUserService.is_authentication(userId);
			UserAuthentication isAuthentication = userAuthenticationService.model(userId,"certification");
	...
	public Response phone_authentication(@RequestParam("userId") int userId,@RequestParam("phone") String phone
		...
		try {
			...
			pd.put("authType", "certification");
			if(!userAuthenticationService.isAuthentication(pd)) {
				return response.failure("请先进行实名认证");
			}
			if(user.getAmount() < 1d) {
			    return response.failure("余额不足，请充值后再试！");
			}
			String message = personalAuthenticationService.phone_authentication(userId,phone,authType);
			if(StringUtils.trim(message).equals("成功")) {
			    String authRemark = user.getNickName() + "于:" + PeriodsUtil.getWholeTime(new Date()) + "进行了手机认证";
			    //保存认证信息
			    UserAuthentication auth = new UserAuthentication("进行了手机认证", authType, 2,phone, authRemark, new Date(), userId);
			    userAuthenticationService.save(auth);
			    //手机认证动态记录
				UserDynamicRecordVO vo = new UserDynamicRecordVO(0, "", "进行了手机认证", DynamicConstants.AUTHENT_CHANGE, "", userId, 0);
				...
				//扣费
				appUserService.user_amount_change(userId, user.getAmount()-1);
				//消费记录
                UserAmountChangeRecord rechargeRecord = new UserAmountChangeRecord(AmountConstants.AUTHENTICATION, AmountConstants.EXPEND, user.getAmount(), (double)1, (double)user.getAmount()-1, userId);
...
//            userExpChangeRecordService.save(record);
	...
	public Response mail_authentication(@RequestParam("userId") int userId,@RequestParam("email") String email,
		...
		try {
			...
			authPd.put("userId", userId);
			authPd.put("authType", "certification");
		    ...
		    j.addProperty("id", userId); 
		    j.addProperty("email", email); 
		    String json = j.toString();    //即为组装好的json字符串。
            String URL = "http://www.xunxinkeji.cn/app-api/personal";
            String token = SymmetricEncoder.AESEncode(KEY_STR,json);
	...
	public ModelAndView activate_mail(@RequestParam("email") String email,@RequestParam("token") String token) {
	    ...
	    String msg = "";
	    try {
	        String jsonObj = SymmetricEncoder.AESDncode(KEY_STR,token.replace(" ", "+"));
	...
	public Response certification_authentication(@RequestParam(value="userId") int userId,@RequestParam(value="authType") String authType,
		...
		try {
			...
			if(result.equals("匹配")) {
			    //修改身份证号
	            appUserService.update_user_certNo(userId,certNo);
			    String authRemark = user.getNickName() + "于:" + PeriodsUtil.getWholeTime(new Date()) + "进行了实名认证.";
			    UserAuthentication auth = new UserAuthentication("进行了实名认证", authType, 2, "", authRemark, new Date(), userId);
			    userAuthenticationService.save(auth);
			    //手机认证动态记录
                UserDynamicRecordVO vo = new UserDynamicRecordVO(0, "", "我新增了实名认证", DynamicConstants.AUTHENT_CHANGE, "", userId, 0);
                userDynamicRecordService.save(vo);
                //赠送积分
                int userExp = user.getUserExp();
                appUserService.user_exp_change(userId, userExp + 50);
...
//            userExpChangeRecordService.save(record);
	...
	public Response sesame_authentication(@RequestParam(value="userId") int userId,@RequestParam("authType") String authType) {
		...
		try {       
			...
			String bizNo = personalAuthenticationService.sesame_initialize(userId);
			String returnUrl = "http://app-api/personal/sesame_authentication_notify";
			String message = personalAuthenticationService.sesame_certify(bizNo,returnUrl);
			if(StringUtils.trim(message).equals("success")) {
			    String authRemark = user.getNickName() + "于:" + PeriodsUtil.getWholeTime(new Date()) + "进行了芝麻认证.";
                UserAuthentication auth = new UserAuthentication("进行了芝麻认证", authType, 2, "", authRemark, new Date(), userId);
                userAuthenticationService.save(auth);
    			//手机认证动态记录
                UserDynamicRecordVO vo = new UserDynamicRecordVO(0, "", "我新增了芝麻认证", DynamicConstants.AUTHENT_CHANGE, "", userId, 0);
                userDynamicRecordService.save(vo);
    			//赠送积分
                int userExp = user.getUserExp();
                appUserService.user_exp_change(userId, userExp + 50);
	 ...
	 * @return
	 */
	@RequestMapping(value=Router.Personal.SESAME_AUTHENTICATION_NOTIFY,method=RequestMethod.POST)
	@ResponseBody
	public Response sesame_authentication_notify(HttpServletRequest request) {
	    log.info("InfoMsg:--- sesame_authentication start");
	    Response response = this.getReponse();
	    try {                                               
	    ...
	    try {
	        ...
	        for(UserAuthentication authent : authentList){
	            if(authent.getAuthType().equals(authentType[0])) {
	                ...
	                pd.put("telephone", authent.getExtendInfo());
	            }
