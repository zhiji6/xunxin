...
public class OperationController extends BaseController{
	...
	public Response sign_In(@RequestParam("phone") String phone,@RequestParam("password") String password) {
		...
		try {
			...
			if(user.getIshiden() == 1){
			    return res.failure("账号已被冻结，请联系客服。");
			}
...
//                int anotherCount = arecordService.findCountByQuery(arecordQuery);
//                
...
//    			int result = arecordCount + anotherCount - matchResult.size();
//    			// 创建一个数值格式化对象     
//                NumberFormat numberFormat = NumberFormat.getInstance();     
//                // 设置精确到小数点后2位     
//                numberFormat.setMaximumFractionDigits(0);     \
//                String proportion = numberFormat.format((float)matchResult.size()/(float)result*100); 
//                
//                pd.put("result", proportion);       //百分比
