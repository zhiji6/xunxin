...
function toUserMsg(toU){
 ...
 function creatw() {
			...
			function send() {
				...
				if(toUser != ""){content = "pegoeadmin886"+toUser+"pegoepegoeadmin888" + content;}
				var message = {};
				if (websocket != null) {
					if (input.getValue()) {
						Ext.apply(message, {
...
Ext.define('MessageContainer', {
	...
	tpl : [
			'<div class="l-im-message-warn">​欢迎使用PEGOE Admin 即时通讯系统。</div>',
			'<tpl for=".">',
