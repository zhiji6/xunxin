...
import com.xunxin.service.XunxinUserDeviceTokenService;
...
public class CommentController extends BaseController{
	 ...
	 */
	@SuppressWarnings("unused")
	@RequestMapping(value=Router.Comment.TO_COMMENT_DETAILS,method=RequestMethod.POST)
	@ResponseBody
	public Response to_comment_details(@RequestParam("commentId") String commentId,Integer userId) {
		log.info("infoMsg:--- 查询评论详情开始");
		Response res = this.getReponse();
		try {
			CommentVO comment =commentService.queryComment(commentId,userId);
			List<CommentVO> commentList =commentService.findReplyList(commentId,userId);
			log.info("infoMsg:--- 查询评论详情结束");
			Map<String, Object> map = new HashMap<>();
			map.put("comment", comment);
			map.put("list", commentList);
			return res.success(map);
		} catch (Exception e) {
			log.error("errorMsg:--- 查询评论详情失败" + e.getMessage());
			return res.failure(e.getMessage());
		}
	}
}
