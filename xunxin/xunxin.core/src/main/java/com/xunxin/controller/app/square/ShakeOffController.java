...
public class ShakeOffController extends BaseController{
	...
	public Response shake_it_off(HttpServletRequest request,@RequestParam("userId") int userId) {
		...
		try {
			...
			Query shakeQuery = new Query()
                    .addCriteria(Criteria.where("userId").is(userId))
                    .addCriteria(Criteria.where("dynamicSource").is(DynamicConstants.SHAKE_OFF));
            UserDynamicRecordVO dynamiRecord = userDynamicRecordService.findOneByQuery(shakeQuery);
            if(dynamiRecord == null) {
                UserDynamicRecordVO userDynamicRecord = new UserDynamicRecordVO();
                userDynamicRecord.setContent("我在广场上玩摇一摇，认识很多有趣的人。");
                userDynamicRecord.setThumbCount(0);
                userDynamicRecord.setDynamicExtend("");
                userDynamicRecord.setDynamicSource(DynamicConstants.SHAKE_OFF);
                ...
                userDynamicRecordService.updateFirst(shakeQuery, update);
            }
