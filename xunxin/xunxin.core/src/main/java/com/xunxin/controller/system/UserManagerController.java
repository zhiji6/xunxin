...
public class UserManagerController extends BaseController{
	...
	public Response user_add_exp() {
	    ...
	    try {
	      ...
	      //生成积分记录
            UserExpChangeRecord record = new UserExpChangeRecord(ExpConstants.ADD_EXP, ExpConstants.INCOME, user.getUserExp(), userExp, user.getUserExp()+userExp, userId);
	 ...
	 * @return
	 */
	@RequestMapping(value=Route.User.WEIXIN_PAY_RETURN)
	public ModelAndView weixin_pay_return(Integer userId,Model model) {
		log.info("跳转支付页面");
		ModelAndView mv = this.getModelAndView();
		System.out.println(userId);
		String nmae = appUserService.findUserNameByUserId(userId);
		model.addAttribute("name", "支付成功");
		mv.setViewName("/userPay");
		return mv;
	}
