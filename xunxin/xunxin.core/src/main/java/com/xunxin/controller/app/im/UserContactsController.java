...
public class UserContactsController  extends BaseController{
	...
	public Response query_friend_interaction(Integer userId,Integer friendId) {
		...
		try {
			...
			Boolean sendpro = selfPortraitService.sendPortrait(userId,friendId);
			if(sendpro){
				map.put("portrait", "Y");
			}else{
				map.put("portrait", "N");
			}
			Boolean message = selfPortraitService.sendMessage(userId,friendId);
			if(message){
				map.put("message", "Y");
			}else{
				map.put("message", "N");
