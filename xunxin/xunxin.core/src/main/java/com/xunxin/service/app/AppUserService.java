...
//		map.put("nickName", user.getNickName());
	...
	public Object getByColum(int userId, String col) throws NoSuchFieldException, SecurityException {
      ...
      //国籍
        String country = "";
        if(user.getNationality() != null && !StringUtils.trim("").equals("")) {
            country =  systemCountryEntityDao.findCountryById(Integer.parseInt(user.getNationality())).getName_Chinese();
        ...
        if(user.getNativePlace() != null) {
            ...
            if(addrs != null && addrs.length > 1) {
                for(String addr : addrs) {
                    nativePlace +=  systemAreaDao.findByCode(addr).getName();
                }
                pd.put("nativePlace", nativePlace);
            }else {
                pd.put("nativePlace", nativePlace);
        ...
        if(user.getCensusRegister() != null) {
            ...
            if(censusRegisters != null && censusRegisters.length > 1) {
                for(String addr : censusRegisters) {
                    censusRegister +=  systemAreaDao.findByCode(addr).getName();
                }
                pd.put("censusRegister", censusRegister);
            }else {
                pd.put("censusRegister", censusRegister);
	...
	public Map<String, Object> queryMatchMessage(Integer userId, Integer anotherUserId) {
		...
		Boolean sendpro = selfPortraitService.sendPortrait(userId,anotherUserId);
		if(sendpro){
			map.put("portrait", "Y");
		}else{
			map.put("portrait", "N");
		}
		Boolean message = selfPortraitService.sendMessage(userId,anotherUserId);
		if(message){
			map.put("message", "Y");
		}else{
			map.put("message", "N");
    ...
    public void update_user_certNo(int userId, String certNo) {
        appUserDao.update_user_certNo(userId,certNo);
    }
    //获取用户消息设置
	public Integer findUserMessageUserId(Integer friendId) {
		return appUserDao.querymessage(friendId);
	}

    public void update_persoanl_status(PageData pd) {
        appUserDao.update_persoanl_status(pd);
    }
