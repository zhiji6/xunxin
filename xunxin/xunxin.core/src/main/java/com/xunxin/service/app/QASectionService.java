...
public class QASectionService extends QASectionDaoImpl{
	...
	//获取板块名称
	public String findTypeName(short type) {
		Query query = new Query();
		query.addCriteria(Criteria.where("sectionType").is(type));
		query.addCriteria(Criteria.where("isDelete").is(false));
		QASection byQuery = findOneByQuery(query);
		if(byQuery != null && byQuery.getSectionName() != null && byQuery.getSectionName().length()>0){
			return byQuery.getSectionName();
		}else{
			return null;
		}
