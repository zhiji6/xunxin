...
public class MapRemoveNullUtil {
  ...
  //根据value值获取到对应的一个key值  
     public static Short getKey(Map<Short, Integer> map,int value){  
       Short key = null;  
       //Map,HashMap并没有实现Iteratable接口.不能用于增强for循环.  
       for(Short getKey: map.keySet()){  
    ...
    public static List<Short> getKeyList(Map<Short, Integer> map,int key){  
        List<Short> keyList = new ArrayList();  
        for(Short getKey: map.keySet()){  
            if(map.get(getKey).equals(key)){  
                keyList.add(getKey);  
            }  
        }  
        return keyList;  
